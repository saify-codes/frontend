...
const page = () => {
    ...
    useEffect(() => {
        if (tab === "1") {
            ...
            setwebsite(draftgigdata?.website);
        }
        if (tab === "2") {
    ...
    const handletab1submit = (e) => {
        ...
        if (gig_id) {
            creategigdraft({ type: "job",website, id: gig_id,  title, positive_tags: tags, price, category, subcategory}, `/seller/createjob?tab=${2}`)
        } else {
            creategigdraft({ type: "job",website,title, positive_tags: tags, price, category, subcategory }, `/seller/createjob?tab=${2}`)
        }
    }
    // Tab 3 
    ...
    const handletab2submit = (e) => {
        ...
        const gig_id = searchParams.get("id");
        if (gig_id) {
            creategigdraft({ id: gig_id, description }, `/seller/createjob?tab=${3}`)
        } else {
            alert("Kindly complete all steps properly")
            window.location = "/seller/createjob?tab=1"
        }
                                    ...
                                    <div className="row create-gig-cat">
                                        ...
                                        <Form.Group controlId="price">
                                                ...
                                                <Form.Control value={website} onChange={(e) => setwebsite(e.target.value)}
                                                    type="text" placeholder="Enter Website URL" required />
                                            </Form.Group>
                                        </div>
