...
import { foodCategories } from '@/Category.js';
...
const page = () => {
    ...
    const categoryNames = foodCategories.map((category) => category.name);
    ...
    const getSubcategories = () => {
        const selectedCategory = foodCategories.find((c) => c.name === category);
        if (selectedCategory) {
    ...
    const handletab1submit = (e) => {
        ...
        if (gig_id) {
            creategigdraft({ type: "food", id: gig_id, deliverytime, title, positive_tags: tags, price, category, subcategory, return: returnp }, `/seller/createfood?tab=${2}`)
        } else {
            creategigdraft({ type: "food", deliverytime, title, positive_tags: tags, price, category, subcategory, return: returnp }, `/seller/createfood?tab=${2}`)
        }
    }
    // Tab 3 
    ...
    const handletab2submit = (e) => {
        ...
        const gig_id = searchParams.get("id");
        if (gig_id) {
            creategigdraft({ id: gig_id, description }, `/seller/createfood?tab=${3}`)
        } else {
            alert("Kindly complete all steps properly")
            window.location = "/seller/createfood?tab=1"
        }
                                    ...
                                    <div className="row create-gig-cat">
                                        <div className="col-md-6 ">
                                            <Form.Group controlId="price">
                                                ...
                                                <Form.Control value={deliverytime} onChange={(e) => setdeliverytime(e.target.value)}
                                                    type="Number" placeholder="Enter Time in Minutes" required />
                                            </Form.Group>
                                        </div>
