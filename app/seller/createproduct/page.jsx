...
const page = () => {
    ...
    const [price, setprice] = useState(draftgigdata?.price)
    ...
    const deliverydays = [
        1, 2, 3, 4, 5, 6, 7, 10, 14, 21, 30, 45, 60
    ]
    const handleCategoryChange = (event) => {
        setcategory(event.target.value);
    };
    const getSubcategories = () => {
        const selectedCategory = productcategories.find((c) => c.name === category);
        if (selectedCategory) {
            setallsubcategories(selectedCategory.products);
        }
        return [];
    };
    useEffect(() => {
    ...
    useEffect(() => {
        if (tab === "1") {
            ...
            setreturnp(draftgigdata?.return)
        }
        if (tab === "2") {
    ...
    const handletab1submit = (e) => {
        ...
        if (gig_id) {
            creategigdraft({type:"product", id: gig_id, deliverytime, title, positive_tags: tags, price, category, subcategory,return:returnp }, `/seller/createproduct?tab=${2}`)
        } else {
            creategigdraft({type:"product", deliverytime, title, positive_tags: tags, price, category, subcategory,return:returnp }, `/seller/createproduct?tab=${2}`)
        }
    }
    // Tab 3 
    ...
    const handletab2submit = (e) => {
        ...
        const gig_id = searchParams.get("id");
        if (gig_id) {
            creategigdraft({ id: gig_id, description}, `/seller/createproduct?tab=${3}`)
        } else {
            alert("Kindly complete all steps properly")
            window.location = "/seller/createproduct?tab=1"
        }
    ...
    return (
        <div>
            <h2 className='text-center my-4'>Create a Product</h2>
                            ...
                            gigloading ? <Loading /> :
                                <Form onSubmit={handletab1submit} className='create-gig-1-form'>
                                    ...
                                    <Form.Group controlId="positiveTags">
                                        ...
                                        <Form.Text className="text-muted">
                                            Enter search terms you feel your buyers will use when looking for your product.
                                            <br />5 tags are max
                                        </Form.Text>
                                    </Form.Group>
                                    ...
                                    <Form.Group controlId="category">
                                        ...
                                        <Form.Control style={{ maxHeight: "4rem", overflow: "auto" }} as="select" value={returnp} onChange={(e) => setreturnp(e.target.value)} required>

                                            <option value="">Select Return Policy</option>
                                                <option key={70} value={7}>
                                                    7 days
                                                </option>
                                                <option key={140} value={14}>
                                                    14 days
                                                </option>
                                        </Form.Control>
                                        <Form.Text className="text-muted">
                                           After how many days a product can be returned to your warehouse
                                            <br />Choose One
                                        </Form.Text>
                                    </Form.Group>
